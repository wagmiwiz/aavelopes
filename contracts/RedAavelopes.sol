...
contract RedAavelopes is ERC721 {
    ...
    mapping (uint256 => uint256) private _originalBalances;
    mapping (uint256 => uint256) private _balances; // scaled balances
    ...
    function mintWithDai(uint256 _amount, uint256 _unlockTimestamp) public returns(uint256) {
        ...
        // Update structures
        _originalBalances[newItemId] += _amount;
        _balances[newItemId] += _amount.rayDiv(lendingPool.getReserveNormalizedIncome(DAI_ADDRESS));
    ...
    function burn(uint256 _tokenId) public {
        ...
        ILendingPool lendingPool = ILendingPool(LENDING_POOL_PROVIDER.getLendingPool());
        lendingPool.withdraw(DAI_ADDRESS, getAmountAsOfNow(_tokenId), msg.sender);

        _originalBalances[_tokenId] = 0;
        _balances[_tokenId] = 0;
        _depositTimestamps[_tokenId] = 0;
        _unlockTimestamps[_tokenId] = 0;
    }

    function getOriginalAmount(uint256 _tokenId) public view returns(uint256)  {
        return _originalBalances[_tokenId];
    }

    function getAmountAsOfNow(uint256 _tokenId) public view returns(uint256)  {
        return _getAmountAsOf(_tokenId, block.timestamp - _depositTimestamps[_tokenId]);
    }

    function getAmountAsOfUnlock(uint256 _tokenId) public view returns(uint256) {
    ...
    function _getAmountAsOf(uint256 _tokenId, uint256 _timePassedInSeconds) internal view returns(uint256) {
        //TODO: add in a calc for future dates approx of interest earned
        ILendingPool lendingPool = ILendingPool(LENDING_POOL_PROVIDER.getLendingPool());
        return _balances[_tokenId].rayMul(lendingPool.getReserveNormalizedIncome(DAI_ADDRESS));
    }
